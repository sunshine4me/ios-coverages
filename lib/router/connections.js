...
function createRouter(opts) {
  ...
  connectionRouter.post("/collect", function (req, res, next) {
  ...
  connectionRouter.use(
    ...
    async (req, res, next) => {
      /** @type {import("../websocket/MyWebSocket")} */
      const connection = req[connectionKey];
      if (connection.lastCollect) {
        req.coverageData = connection.lastCollect.coverageData;
        const version = await coverageService.getVersion(connection.versionName);
        if (version) {
          req.objectFilePath = version.objectFilePath;
        }
        next();
      } else {
        return res.error("lastCollect not found");
      }
    },
    iosAnalysisRouter
  );
